...
const actions = {
    async readMedicaments({
        ...
        reader.onload = e => {
            ...
            dispatch("uploadMedicament", {json: json, image: file.image})
        };
    ...
    async uploadMedicament({
        ...
        const medicationRef = await addDoc(collection(db, "Medikamente"), file.json);
          dispatch("uploadMedicamentPicture", {id: medicationRef.id, image: file.image})
    },
    async uploadMedicamentPicture({
        ...
        dispatch("updateMedicamentPicture", {name: name, id: data.id})
    },
