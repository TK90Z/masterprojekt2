<template>
  <v-card v-if="rights == 3 || rights == 4">
    <v-data-table
      ...
      sort-by="uid"
      group-by="rights"
      class="elevation-1 light-blue lighten-5"
      show-group-by
    >
      ...
      <template v-slot:item.actions="{ item }">
        ...
        <v-icon
          small
          class="mr-2 stopEditIcon"
          @click="stopEditMode(item)"
          v-if="item.editMode"
        >
          mdi-cancel
        </v-icon>
      ...
      <template v-slot:item.rightsAutocomplete="{ item }">
        <p></p>
        <v-autocomplete
          v-model="item.editRights"
          @change="editRightsChanged(item)"
          :items="rechte"
          dense
          label="Rechte"
          item-text="text"
          item-value="index"
          :disabled="autoDisable(item)"
        >
        </v-autocomplete> </template
    ></v-data-table>
    <v-dialog
      v-model="subjectAreaDialog"
      ...
      <template>
        <v-card class="light-blue lighten-5">
          <v-toolbar color="primary" dark
            >Bitte wählen Sie eine Fachrichtung aus.</v-toolbar
          >
          <v-card-text>
            <v-select
              v-model="subjectAreaItems"
              :items="subjectAreas"
              item-text="text"
              item-value="index"
              :menu-props="{ maxHeight: '400', offsetX: true, right: true }"
              multiple
              hint="Wählen Sie die Fachrichtung(en) aus"
              persistent-hint
            ></v-select>
          </v-card-text>
          <v-card-actions class="justify-end">
            <v-btn
              @click="subjectAreaDialog = false"
    ...
    <v-dialog
      v-model="patientCredentialsDialog"
      transition="dialog-top-transition"
      max-width="600"
      persistent
    >
      <template>
        <v-card class="light-blue lighten-5">
          <v-toolbar color="primary" dark
            >Bitte tragen Sie weitere Informationen zum Patienten ein.
          </v-toolbar>
          <v-card-text>
            <p></p>
            <v-text-field
              v-model="patientCredentials.age"
              label="Alter"
            ></v-text-field>
            <v-select
              v-model="patientCredentials.sex"
              :items="sexes"
              :menu-props="{ maxHeight: '400', offsetX: true, right: true }"
              label="Geschlecht"
              persistent-hint
            ></v-select>
            <v-text-field
              v-model="patientCredentials.insurance"
              label="Versicherung"
            ></v-text-field>
          </v-card-text>
          <v-card-actions class="justify-end">
            <v-btn
              @click="patientCredentialsDialog = false"
              class="abbrechen-buttons"
              rounded
              outlined
              text
            ...
            <v-btn
              ...
              outlined
              text
              @click="changePatientCredentials"
              >Bestätigen</v-btn
            >
          </v-card-actions>
        </v-card>
...
export default {
  ...
  methods: {
    changePatientCredentials() {
      if (
        this.patientCredentials.age &&
        this.patientCredentials.sex &&
        this.patientCredentials.insurance
      ) {
        this.patientCredentialsDialog = false;
      }
    },
    stopEditMode(item) {
      (item.editMode = false), (item.editRights = item.rights);
